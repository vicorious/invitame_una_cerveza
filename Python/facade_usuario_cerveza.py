...
class UserBeerFacade:
 ...
 ############ Update beer for visit ####################################################
    def user_beer_for_visit(self, _json):
        ...
        try:
            _json_entrada = json.loads(_json)
            results = self.cursor.beer_connection.session.query(UserBeer).filter(UserBeer.user_id == _json_entrada["user_id"], UserBeer.beer_id == _json_entrada["beer_id"])
            return results
        except Exception as _excep:
            logging.debug('Exception: %s"', _excep)
        finally:
 ...
 ############ getUserBeerForPayment ####################################################
    def user_beer_pay_type_for_visit(self, _json):
        ...
        try:
            _json_entrada = json.loads(_json)
            results = self.cursor.beer_connection.session.query(UserBeer).filter(UserBeer.user_id == _json_entrada["user_id"], UserBeer.beer_id == _json_entrada["beer_id"], UserBeer.pay_type_id == _json_entrada["pay_type_id"])
            return results
        except Exception as _excep:
            logging.debug('Exception: %s"', _excep)
        finally:
 ...
 ############ getUserPayType ####################################################
    def user_pay_type_for_visit(self, _json):
        ...
        try:
            _json_entrada = json.loads(_json)
            results = self.cursor.beer_connection.session.query(UserBeer).filter(UserBeer.user_id == _json_entrada["user_id"], UserBeer.pay_type_id == _json_entrada["pay_type_id"])
            return results
        except Exception as _excep:
            logging.debug('Exception: %s"', _excep)
        finally:
